...
def transform(**context):
    ...
    for day in data['daily']:
        weather_list.append(tuple([datetime.fromtimestamp(day['dt']).strftime('%Y-%m-%d'), day['temp']['day'], day['temp']['min'], day['temp']['max']]))
    return weather_list

def load(**context):
    schema = context['params']['schema']
    table = context['params']['table']
    weather_list = context['task_instance'].xcom_pull(key='return_value', task_ids='transform')
